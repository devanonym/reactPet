...
const apiUrl = SERVER_API_URL + '/api/pets';
...
export const getEntities: ICrudGetAllAction<IPet> = (page, size, sort) => {
  ...
  return {
    ...
    payload: axios.get(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`) as Promise<IPet>
  };
};
...
export const getEntity: ICrudGetAction<IPet> = id => {
  ...
  return {
    ...
    payload: axios.get(requestUrl) as Promise<IPet>
  };
};
